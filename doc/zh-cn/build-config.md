...
将 `std/` 目录下的标准库模块（`.psm1`）纳入构建。

| 字段 | 类型 | 必选 | 默认值 | 说明 |
| - | - | - | - | - |
| `Enable` | `bool` | 否 | `true` | 是否启用标准库构建，设为 `false` 可跳过 |

标准库文件通过 `## Module` 和 `## Import` 预处理指令声明模块名和依赖关系，构建器会自动进行拓扑排序。
...
> **注意**：标准库源文件位于 `infinity_build/std/` 目录下。若 `Enable` 为 `false` 或缺少该配置，标准库模块将不会被包含在构建产物中。
